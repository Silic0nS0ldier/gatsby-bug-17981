...
}) =>
{
  ...
  `);
    ...
    posts.forEach(({ node, }, index) =>
    {
        ...
        // Only show completed pages in production
    ...
    const tags = new Set(
        result.data.allMarkdownRemark.edges.map(edge => get(() => edge.node.frontmatter.tags, [])).flat(),
    );
    tags.forEach((tag) =>
    {
        createPage({
    ...
    {
        createPage({
            path: `/author/${kebabCase(edge.node.id)}/`,
            component: authorTemplate,
            context: { author: edge.node.id, },
        });
    });
};
